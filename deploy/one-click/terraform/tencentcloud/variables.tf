...
variable "tke_node_count" {
  description = "Desired node count of the optional TKE node pool. 0 disables the extra node pool; initial cluster workers are controlled by worker_config.count."
  type        = number
  default     = 0

  validation {
    condition     = var.tke_node_count >= 0 && floor(var.tke_node_count) == var.tke_node_count
    error_message = "tke_node_count must be an integer >= 0."
  }
}
