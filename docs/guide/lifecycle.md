...
- **Cluster coordination**: auto-pause is driven by `cube-proxy-sidecar`, co-resident with each CubeProxy container. It consumes lifecycle events CubeMaster publishes via Redis stream and maintains sandbox state accordingly.
- **Replica mode (important)**: auto-pause/auto-resume is currently **only guaranteed correct when CubeProxy runs as a single replica**, which is the default deployment shape (`cube_proxy_replicas=1`). Each CubeProxy's sidecar sweeper decides whether a sandbox is idle using only the last-active timestamps of requests that hit **its own replica**. If CubeProxy is scaled to multiple replicas behind a round-robin / least-conn LB, requests for a single sandbox are spread across replicas, so no individual sidecar sees the full activity stream — **a replica that did not serve recent requests will believe the sandbox is idle and pause it**, producing a "pause → auto-resume" churn loop.
- **Prerequisite for multiple replicas**: if you must scale CubeProxy to more than one replica (for HA/throughput), the **front-end LB MUST hash on SandboxID** (session affinity / consistent hashing) so that all traffic for a given sandbox is pinned to a single replica; otherwise auto-pause/auto-resume will misfire.
